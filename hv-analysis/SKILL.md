...
信息收集完成后、动笔写报告之前，必须先把研究素材结构化为 JSON 底稿，保存到工作目录：`[研究对象]_研究底稿.json`。底稿结构遵循本Skill自带的 `references/schema.json`——这份 schema 就是底稿的字段定义：纵轴、横轴、交汇各需要填什么，一目了然。

硬性要求：

- 底稿中的每个关键事实（时间线节点、竞品信息、用户口碑等）必须在 `来源` 字段标注来源编号 `[S1]`、`[S2]`……
- 所有来源统一登记在底稿的 `信息来源注册表`：`编号 / 标题 / 发布方 / URL / 访问日期 / 支撑的事实`。找不到来源的事实如实标注「暂缺」，绝不编造 URL
- 后续写正文时沿用同一套编号，在关键事实后以 `[S编号]` 标注；文末「信息来源」一节按编号列出完整清单
- 底稿是正式交付物，随报告一起保留：报告里任何一句话都能回溯到具体来源，二次研究时也能在底稿上增量更新而不是推倒重来
...
Markdown 完稿后，先运行本Skill自带的质检脚本，通过后才能转 PDF：

```bash
python [skill目录]/scripts/verify_report.py [研究对象]_横纵分析报告.md --research [研究对象]_研究底稿.json
```

脚本做的硬校验（不是自检清单那种软约束）：

- 必备章节是否齐全（一句话定义/纵向/横向/交汇/信息来源）
- 引证是否闭环：正文每个 `[S编号]` 必须在来源清单有登记，登记的来源必须带 URL 或「暂缺」标注，正文不允许零引证
- 有没有占位符、假链接
- 字数是否在 10000-30000 区间
- 有没有触犯「绝对禁区」的AI味禁词（逐行报出）

有 ERROR 必须修复后重跑，全部通过（0 ERROR）才能进入转换流程。
...
在Markdown稿件末尾加上：
- **信息来源**：按编号列出所有引用的来源，每条格式为 `[S编号] 标题 — 发布方，URL（访问日期）`，与研究底稿的注册表保持一致；正文中关键事实后需有对应的 `[S编号]` 引注
...
1. `[研究对象]_横纵分析报告.pdf` — 最终报告
2. `[研究对象]_横纵分析报告.md` — Markdown 源稿（PDF 的底本）
3. `[研究对象]_研究底稿.json` — 结构化研究素材 + 信息来源注册表（引证持久化产物，供复查和二次研究）
...
- [ ] 没有触犯绝对禁区里的任何一条？
- [ ] 所有关键事实以 `[S编号]` 标注了来源，且与来源清单一一对应？
- [ ] 研究底稿 JSON 已保存，来源注册表完整、无编造 URL？
- [ ] `verify_report.py` 已运行且 0 ERROR 通过？
- [ ] 搜不到的信息诚实标注了「暂缺」，没有编造？
- [ ] PDF排版美观、结构清晰、可读性好？
